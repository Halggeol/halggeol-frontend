...
// ✅ 데이터 로드 함수 - type 파라미터를 전달하지 않음 (모든 상품 조회)
async function loadData(round, productId) {
  ...
  try {
    const {
      ...
      round: r,
      productId: pid,
    } = await getInsightList(round, productId); // type 파라미터 제거
    ...
    if (dData.forexInfo?.length > 0 && dData.currency) {
      const currencies = dData.currency.split(',').map(c => c.trim());
...
// ✅ URL 쿼리 변경 감지
watch([() => route.query.round, () => route.query.productId], ([newRound, newProductId]) => {
  const round = newRound ? Number(newRound) : null;
  const productId = newProductId || null;
  
  if (round !== currentRound.value || productId !== currentProductId.value) {
    loadData(round, productId);
  }
}, { immediate: false });
     ...
     <div class="header-navigation">
      ...
      <div class="main-content">
        ...
        <div class="products-sidebar">
          ...
          <div v-else>
            ...
            >
              ...
              <span class="product-name">{{ product.name || (idx === 0 ? '이스트스프링디클로벌리더스 40증권자투자신탁[제한환급]' : '한국투자증권 글로벌 ETF 연금') }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="chart-area">
          ...
          <!-- 외환 상품인 경우 -->
          <div v-else-if="isForexProduct" class="forex-section">
